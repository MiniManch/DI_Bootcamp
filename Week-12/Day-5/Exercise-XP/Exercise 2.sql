...
SELECT concat(first_name,' ',last_name) as fullname from customer limit 10;
...
select * from customer order by first_name desc limit 10;
...
--  Have your query get the film ID, title, description, length and the rental rate, these details can be found in the “film” table.
 select COUNT(title) from film where title like '%deadpool%';
 -- select film_id,title,description,length,rental_rate where title like '%deadpool%';
...
-- select  * from( select * from film order by rental_rate asc limit 20) as first_20  limit 10;
select * from film order by rental_rate asc limit 20,10;
...
SELECT customer.customer_id, first_name ,last_name,payment.amount,payment.payment_date 
FROM customer 
INNER JOIN payment where customer.customer_id = payment.customer_id 
...
FROM city
INNER JOIN country WHERE country.country_id = city.country_id;
...
SELECT *  from payment where staff_id = 2; -- there is only 2 staff_id and too many results.